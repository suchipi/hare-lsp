...
| `tags`                             | string[] | `[]`        | Build tags (`-T <tag>` pairs) forwarded to every `hare {build,test,run}` invocation the server spawns.                                                                                                                                                                                                                                                                                          |
| `extraArgs.build`                  | string[] | `[]`        | Extra arguments appended to the on-save `hare build` type-check pass. Each element is one argv slot (no shell splitting). e.g. `["-lc"]` to link libc.                                                                                                                                                                                                                                          |
| `extraArgs.test`                   | string[] | `[]`        | Extra arguments appended to `hare test` invoked via the test codelens.                                                                                                                                                                                                                                                                                                                          |
| `extraArgs.run`                    | string[] | `[]`        | Extra arguments appended to `hare run` invoked via the run codelens.                                                                                                                                                                                                                                                                                                                            |
| `diagnostics.debounceMs`           | number   | `300`       | Minimum ms between the last `didChange` and the next parse-diagnostics refresh.                                                                                                                                                                                                                                                                                                                 |
| `diagnostics.enableBuild`          | boolean  | `true`      | Run `hare build` on save to surface type-check diagnostics (undefined names, type mismatches, etc.) on top of the in-process parse pass. Disable to skip the build pass; type errors then won't show up until you build manually.                                                                                                                                                               |
...
| `formatting.tabWidth`              | number   | `null`      | Overrides the display columns per indentation tab used for line-wrap decisions, taking precedence over the editor's `tabSize` from the formatting request. When `null` (unset), the formatter uses the request's `tabSize` (falling back to 8). Indentation is always emitted as literal tabs regardless. Lets collaborators who render tabs at different display sizes wrap to the same width. |
| `hover.useHtml`                    | boolean  | `true`      | Allow inline HTML tags in hover markdown so HTML-aware clients (VSCode) can style content (e.g. fine print). Disable for editors that don't render HTML in hover markdown (Neovim, Helix, Emacs default renderers) to avoid showing literal tags.                                                                                                                                               |
...
This affects only where lines wrap; indentation is always emitted as literal tabs regardless of the value. Through the LSP the server uses the editor's `tabSize` from the formatting request by default (the LSP spec makes it a required field), so wrapping matches what you see on screen; `--tab-width` is the CLI equivalent of that setting. Set [`hare.formatting.tabWidth`](#configuration) to pin a fixed wrap width that overrides `tabSize`, so collaborators who render tabs at different display sizes still wrap to the same width.
