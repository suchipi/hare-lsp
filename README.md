...
- **Workspace indexing is chunked, not concurrent.** Hare is single-threaded, so indexing runs cooperatively between LSP messages: each dispatch tick processes a small batch and yields back. Progress is reported via `$/progress` and the job can be cancelled with `window/workDoneProgress/cancel`. Caveat: a fully idle editor (sending no messages) won't drive the job forward; typing or a pull-diagnostics request unblocks it. `workspace/symbol` results may carry `isIncomplete: true` while the job is still draining.
