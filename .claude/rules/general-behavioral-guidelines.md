...
**Scope is set by the request, not by this guideline.** "Minimum code that solves the problem" means _the problem as stated_ — not a paring-down of it. If the task is inherently large (implementing a feature spec, completing a migration, executing a multi-step plan), do the whole thing. Do not invoke "simplicity" or "minimum code" as a reason to deliver less than what was asked. Skipping steps, dropping requirements, or stopping early because the work feels big is laziness, not simplicity.
