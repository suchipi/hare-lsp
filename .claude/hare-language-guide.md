...
- **`loc.end.off` is the start of the LAST RUNE of the last consumed token, not the end of that token.** This comes from `hare::lex::prevloc`. For `b.x`, `loc.end.off` of the access*field equals the start of `x` — and \_also* equals the end of `x`, because `x` is a single rune. For `b.data` it points at `a` (start of the last rune of `data`), not at `d` and not past `a`. Tests that use single-character names will not catch a mishandling of this. See the "Byte / rune offsets" section of [CLAUDE.md](../CLAUDE.md).
