...
- `parse.ha.patch`: fixes `want()`, which in stdlib aborts the whole process ("attempted to unlex more than one token") on intermediate editing states. The patched copy returns an error instead.
