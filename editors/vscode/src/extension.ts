      ...
      executeCommand: async (command, args, next) => {
        if (command === "hare-lsp.runTest") {
          ...
          const parser = createTestOutputParser({
            ...
            onFailure: (failure) => {
              recordTestFailure(uri, failure.testName, {
                uri: vscode.Uri.file(failure.absPath).toString(),
                line: failure.line,
                col: failure.col,
    ...
    vscode.window.onDidStartTerminalShellExecution(async (event) => {
      ...
      const parser = createTestOutputParser({
        ...
        onFailure: (failure) => {
          ...
          if (idx) applyFailure(failure.testName, loc);
          else pendingFailures.push({ name: failure.testName, loc });
        },
      });
